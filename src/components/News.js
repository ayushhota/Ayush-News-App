...
//                     <h1>AyushNewsApp - Top Headlines</h1>
//                     <div className="row">
...
// import PropTypes from 'prop-types' // "impt" is the snippet used here to generate this line 
...
export class News extends Component {
    ...
    // if we are using "componentDidMount" then there is no need to take this article variable
    // articles = [
    //     {
    //         "source": {
    //             "id": "cnn",
    ...
    // ******React 27*******
    //     constructor() {
    //         super(); // this is the syntax for the constructor and it is mandatory
    //         console.log("we are inside the constructor");
    //         this.state = { // this is the also the syntax which is present in the documentation of "react.org"
    ...
    //         return (
    //             <div className="container my-3">
    //                 <h1 className='my-3'>AyushNewsApp - Top Headlines</h1>
    //                 <div className="row">
    //                     {this.article.}
    //                     <div className="col-md-4">
    //                         <NewsItem title="myTitle" description="myDescription" imageUrl="https://i.guim.co.uk/img/media/0e1a2d307ec034dda2736bd96ee5c1734236583b/0_345_5184_3110/master/5184.jpg?width=1200&height=630&quality=85&auto=format&fit=crop&overlay-align=bottom%2Cleft&overlay-width=100p&overlay-base64=L2ltZy9zdGF0aWMvb3ZlcmxheXMvdGctZGVmYXVsdC5wbmc&enable=upscale&s=f140e725fc7f4eea3ef1944c69f4c165" />
    //                     </div>
    //                     <div className="col-md-4">
    //                         <NewsItem title="myTitle" description="myDescription" />
    //                     </div>
    ...
    constructor(props) {
        super(props); // this is the syntax for the constructor and it is mandatory
        console.log("we are inside the constructor");
        this.state = { // this is the also the syntax which is present in the documentation of "react.org"
            // articles: this.articles,

            articles: [], // for "React 29"
            loading: true,
            page: 1
        }
        document.title = `${this.props.category}-AyushNewsApp`
    }


    // ******React 29*******     (remember this as it is important)
    // async componentDidMount(){      // this will run only when render finishes its work
    //     let url = "https://newsapi.org/v2/top-headlines?country=${this.props.country}&category=${this.props.category}&apiKey=fa44577939ad42719181e3d1e2bdce12";
    //     let data = await fetch(url);
    //     let pasrseData = await data.json();
    //     console.log(pasrseData);
    //     this.setState({articles: pasrseData.articles})
    // }



    // ******React 30*******     (remember this as it is important)

    async componentDidMount() {      // this will run only when render finishes its work 
        
        this.props.setProgress(30); // ******React 38*******(all set progress things are from 38th lecture)
        let url = `https://newsapi.org/v2/top-headlines?country=${this.props.country}&category=${this.props.category}&apiKey=fa44577939ad42719181e3d1e2bdce12&page=1&pageSize=${this.props.pageSize}`; // "&page=1" is the syntax in url to show the news of the respective pages
    ...
    handlePreviousClick = async () => {
        ...
        // let url = `https://newsapi.org/v2/top-headlines?country=${this.props.country}&category=${this.props.category}&apiKey=fa44577939ad42719181e3d1e2bdce12&page=${this.state.page - 1}&pageSize=${this.props.pageSize}`; // "&page=1" is the syntax in url to show the news of the respective pages
        // let data = await fetch(url);
        // let pasrseData = await data.json();
        // console.log(pasrseData);
        // this.setState({
    ...
    handleNextClick = async () => {
        ...
        //     console.log("Next button will not work");
        // }
    ...
    render() {
        return (
            <div className="container my-3">
                <h1 className='text-center my-3' style={{ padding: '70px 0px 30px 0px' }}>AyushNewsApp - WELCOMES YOU</h1>
                <h1 className='text-center'>{this.props.category} Category</h1>


                {/* ********React 31************ */}

                {this.state.loading && <Spinner />} {/* this is the js syntax which says that "if loading in true than show the loading gif otherwise dont show" */}

                <div className="row my-3">
                    {/* here is also the react 31 lecture syntax is present */}
                    {!this.state.loading && this.state.articles.map((element) => {
                        return <div className="col-md-4" key={element.url}> {/* "return" is mandatory */}
